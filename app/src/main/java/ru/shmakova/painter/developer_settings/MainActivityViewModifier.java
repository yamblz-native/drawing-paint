...
public class MainActivityViewModifier implements ViewModifier {
    ...
    public <T extends View> T modify(@NonNull T view) {
        ...
        DrawerLayout drawerLayout = (DrawerLayout) view.findViewById(ru.shmakova.painter.R.id.main_drawer_layout);

        DrawerLayout.LayoutParams layoutParams = new DrawerLayout.LayoutParams(MATCH_PARENT, MATCH_PARENT);
        layoutParams.gravity = Gravity.END;

        drawerLayout.addView(LayoutInflater.from(view.getContext()).inflate(ru.shmakova.painter.R.layout.developer_settings_view, drawerLayout, false), layoutParams);
        return view;
    }
}
